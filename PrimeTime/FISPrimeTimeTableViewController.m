...
- (void)generatePrimeNumbers{
    NSInteger number = 0;
    while ([self.primeNumbers count] < 10000) {
        if ([self isPrime:number] && number > 1) {
            [self.primeNumbers addObject:@(number)];
        }
        number++;
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"PrimeNumberCell" forIndexPath:indexPath];
    
    cell.textLabel.text = [NSString stringWithFormat:@"%@", [self.primeNumbers objectAtIndex:indexPath.row + 5000]];
